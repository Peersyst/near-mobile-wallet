...
describe("TransactionStatus tests", () => {
    test("Renders pending", () => {
        const screen = render(<TransactionStatus status={TransactionStatusType.PENDING} variant="body3Regular" />);
        expect(screen.getByText(translate(TransactionStatusType.PENDING))).toBeDefined();
    });

    test("Renders proposed", () => {
        const screen = render(<TransactionStatus status={TransactionStatusType.PROPOSED} variant="body3Regular" />);
        expect(screen.getByText(translate(TransactionStatusType.PROPOSED))).toBeDefined();
    });

    test("Renders committed", () => {
        const screen = render(<TransactionStatus status={TransactionStatusType.COMMITTED} variant="body3Regular" />);
        expect(screen.getByText(translate(TransactionStatusType.COMMITTED))).toBeDefined();
    });

    test("Renders rejected", () => {
        const screen = render(<TransactionStatus status={TransactionStatusType.REJECTED} variant="body3Regular" />);
