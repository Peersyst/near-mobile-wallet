...
import * as useUpdateUncommittedTransactions from "module/transaction/hook/useUpdateUncommitedTransactions";
...
describe("useAddUncommittedTransaction", () => {
    test("Works correctly when adding one tx without previous", () => {
        ...
        const mockedUseUpdateUncommittedTransactions = jest.fn();
        jest.spyOn(useUpdateUncommittedTransactions, "default").mockReturnValue(mockedUseUpdateUncommittedTransactions);
        ...
        expect(mockedUseUpdateUncommittedTransactions).toHaveBeenCalledWith(["hash"], 0);
    ...
    test("Works correctly when adding one tx with previous txs", () => {
        ...
        expect(addUncommitedTx).toBeDefined();
        addUncommitedTx(0, Chains.TESTNET, "hash");
        expect(mockedUseUpdateUncommittedTransactions).toHaveBeenCalledWith(["hash1", "hash"], 0);
        expect(mockedAddUncommittedTransactionHash).toBeCalledWith(0, Chains.TESTNET, "hash");
        addUncommitedTx(1, Chains.TESTNET, "hash");
        expect(mockedUseUpdateUncommittedTransactions).toHaveBeenLastCalledWith(["hash2", "hash"], 1);
