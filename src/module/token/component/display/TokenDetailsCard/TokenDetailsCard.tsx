...
const TokenDetailsCard = ({ token }: TokenDetailsCardProps): JSX.Element => {
    ...
    return (
        <TokenDetailsCardRoot>
            ...
            />
            <Row gap={8}>
                <TokenDetailsCardButton size="lg" onPress={handleOnSendButtonPress} disabled={!haveNearInAccount}>
                    {capitalize(translate("send"))}
                </TokenDetailsCardButton>
                <TokenDetailsCardButton size="lg" onPress={handleOnSwapButtonPress} disabled={!haveNearInAccount}>
