...
export const WalletStorage = new (class extends BaseStorageService<SecureWalletStorageType, UnsecureWalletStorageType> {
    ...
    async addWallet(wallet: StorageWallet, network: NetworkType, privateKey: string, index: number): Promise<void> {
        ...
        await this.addSecureWalletId(index, privateKey, network);
    }
    ...
    async addSecureWalletId(walletId: number, privateKey: string, network: NetworkType): Promise<void> {
        await this.addSecureWalletIds([walletId], privateKey, network);
    }

    async addSecureWalletIds(walletIds: number[], privateKey: string, network: NetworkType): Promise<void> {
        ...
        if (walletGroup) {
            ...
            await this.setSecureWallets([...secureWallets, { privateKey, walletIds }], network);
        }
    }
