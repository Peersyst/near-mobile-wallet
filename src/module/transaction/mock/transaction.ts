import { Transaction, TransactionStatus, TransactionType } from "../types";

export const transaction: Transaction = {
    status: "committed" as TransactionStatus as TransactionStatus,
    transactionHash: "0xa43e1c45350cdcf16bfd0e5dda832302cf34300054f1ddeb571cf6ce178bb700",
    inputs: [],
    outputs: [],
    blockHash: "0x36d810d978e50426f52b29d36e3a1e7a9a1eb14a9ece2f30da16e9c81c61398b",
    blockNumber: 4821477,
    type: "send_ckb" as TransactionType,
    amount: 10250,
    timestamp: new Date("2022-03-25T10:59:05.094Z"),
};

export const transactions: Transaction[] = [
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x8997875ae2f4b1e42e5a894dd2a2ba147bf2f4c9e0e58ddf4a6dff38b4a254be",
        inputs: [],
        outputs: [],
        blockHash: "0xb83e97ab314cf6980385a0d9bdb885d3b380e85967a913b00dcb7376ec380a83",
        blockNumber: 4425606,
        type: "receive_ckb" as TransactionType,
        amount: 10000,
        timestamp: new Date("2022-02-16T18:39:35.639Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xdf1c8ca38b048e43b586fce520a5fff6aa3b0645f9279a6a7f42f9e872673b53",
        inputs: [],
        outputs: [],
        blockHash: "0x993f6dce66b38248b5561427a2257a576bed8a72874a311b63079c70952dcea0",
        blockNumber: 4426286,
        type: "receive_ckb" as TransactionType,
        amount: 75,
        timestamp: new Date("2022-02-16T20:09:53.108Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x6ecef55d8e4e84996180e13561d4d40ab5ad6a08843d9ff27ebe8c3b72d6d6ca",
        inputs: [],
        outputs: [],
        blockHash: "0x5ed08ffd2c2900f5d03c6285225c01409ca36ac775aec6ac2e6a5a1418b7883f",
        blockNumber: 4434312,
        type: "receive_ckb" as TransactionType,
        amount: 150,
        timestamp: new Date("2022-02-17T14:00:18.514Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xe1a7cf802ef39c5395262f523962a5b4acee1292f4c51a92683604a59f991257",
        inputs: [],
        outputs: [],
        blockHash: "0x301f81ea042ae18f8f62bbfb1b73bb411aebd1d649d2fd8501a8230255975ed8",
        blockNumber: 4435113,
        type: "receive_ckb" as TransactionType,
        amount: 123,
        timestamp: new Date("2022-02-17T15:44:06.256Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x1a50505b2a0a7e8eebb06fda96abcbff6551f2b5711ba8266fd4a6f35833aa05",
        inputs: [],
        outputs: [],
        blockHash: "0x17a505353a671db2874b8a789a2706cab69579919ea577fe893d3a1686b211a4",
        blockNumber: 4479608,
        type: "deposit_dao" as TransactionType,
        amount: 1000,
        timestamp: new Date("2022-02-21T18:48:21.881Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x992208eab19d0f8ce5a2fc10579d8d614d265aa12851ea140ec717f2f41b925f",
        inputs: [],
        outputs: [],
        blockHash: "0x4b3422f42a72c313c74d4b1c15a89fa16d466f8602dea9cd097cf0544177356d",
        blockNumber: 4479744,
        type: "deposit_dao" as TransactionType,
        amount: 150,
        timestamp: new Date("2022-02-21T19:03:28.714Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x805168dafc0c10ae31de2580541db0f5ee8ff53afb55e39a5e2eeb60f878553f",
        inputs: [],
        outputs: [],
        blockHash: "0x7895667ef62cf0b4bc7be9485577d7feaf9c278fd1b50e9f66cdc3292cf32335",
        blockNumber: 4498929,
        type: "withdraw_dao" as TransactionType,
        amount: 1000,
        timestamp: new Date("2022-02-23T13:33:31.920Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x6d22619e2866924f585b440543927bb4d21b8bdfac6e415fa156fc66f6a97af0",
        inputs: [],
        outputs: [],
        blockHash: "0xdfd5fe561c5daeff59601bdd0673ec912148b5c13421aa9411624c93c15e0daf",
        blockNumber: 4511013,
        type: "deposit_dao" as TransactionType,
        amount: 5000,
        timestamp: new Date("2022-02-24T16:22:20.642Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x87c3586cc91ec7a1a97407456dba0adb34c5781c6031cb71bfb175939af4e6c6",
        inputs: [],
        outputs: [],
        blockHash: "0xbbb1f70add4698b3b9f2ee1d475a1c391c4337c135680169269ca61afacd00fc",
        blockNumber: 4519050,
        type: "deposit_dao" as TransactionType,
        amount: 500,
        timestamp: new Date("2022-02-25T10:28:29.306Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x62e2b992ca219b27e7c508e65c0bacd92403a467a213cbc1d148d09cd8a553ae",
        inputs: [],
        outputs: [],
        blockHash: "0x8ed8e88a9eb337e453ca6937ebc010676689037e7d1b93f8b9bc0ac155e61a05",
        blockNumber: 4519481,
        type: "receive_ckb" as TransactionType,
        amount: 10000,
        timestamp: new Date("2022-02-25T11:23:47.510Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xda24328c67b182c0354d76385e952451ffb8964bc447ef69fdade5e20cbb1dd8",
        inputs: [],
        outputs: [],
        blockHash: "0x569e198c49fad75e0f30545aedd1f6da5c1972ca1199577bbb3cd788c31a207e",
        blockNumber: 4553464,
        type: "receive_token" as TransactionType,
        amount: 142,
        timestamp: new Date("2022-02-28T14:52:53.713Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x54658a3934b8b5c2bc7a68c5f118f2509dbdd2cd64f3d24c75901c2ba74cb026",
        inputs: [],
        outputs: [],
        blockHash: "0x75c679dd2e3bd40f0836daa8f60c0af9f2a9a6f76898639bb1c5261a2cb7a018",
        blockNumber: 4553477,
        type: "receive_token" as TransactionType,
        amount: 142,
        timestamp: new Date("2022-02-28T14:54:52.556Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x45ae27991ea926a28bd0ef0d3dbbb2350fb1dc3bceff6de4272c4a00027d36a0",
        inputs: [],
        outputs: [],
        blockHash: "0x203e6bded8043383ff19db72c2947d8e18e3ca843be16f0205f861c089ab09fd",
        blockNumber: 4572537,
        type: "receive_ckb" as TransactionType,
        amount: 10000,
        timestamp: new Date("2022-03-02T09:23:53.839Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xe3132f95229695bc2d6f533dce9e165142d9bb018fd6673c61b029bf079f8608",
        inputs: [],
        outputs: [],
        blockHash: "0x4881ac694de5bb3f6be5258cc1085514c472cb9dacc4798ab0bc4561b49e8485",
        blockNumber: 4582797,
        type: "deposit_dao" as TransactionType,
        amount: 9000,
        timestamp: new Date("2022-03-03T08:19:17.057Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x9f0cf27529cc2a7253e709072540976d94b2756375b355bf01b8e4fc6a2a3e11",
        inputs: [],
        outputs: [],
        blockHash: "0x6ffc4841a709a9de597a7bf480044c3a88b4e9a6281c4184a67714319ea3574b",
        blockNumber: 4582890,
        type: "deposit_dao" as TransactionType,
        amount: 12000,
        timestamp: new Date("2022-03-03T08:33:34.858Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x57ae2d2c42eaaf6d1ca6d6b501cfb7ea6001dac3a0529ee6654651f2298f1ddc",
        inputs: [],
        outputs: [],
        blockHash: "0x7d4369400be0f550a36fe53663d88977b0b5cfd4135457b18fda8912c9a66f97",
        blockNumber: 4648760,
        type: "deposit_dao" as TransactionType,
        amount: 500,
        timestamp: new Date("2022-03-09T10:39:39.408Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x243dbedc0dccba7c04a867014cf744810f2937ccc7c0ce54d7e558de566e2121",
        inputs: [],
        outputs: [],
        blockHash: "0x42c055d3f2515413476e82a5aa8c840be959d793dfc7180ad8b44c986bccc69b",
        blockNumber: 4648834,
        type: "deposit_dao" as TransactionType,
        amount: 500,
        timestamp: new Date("2022-03-09T10:49:01.409Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x9469fc7c6991197ee6f370244402b75cba78360fa4d553ad082fbb0dae2d7626",
        inputs: [],
        outputs: [],
        blockHash: "0xdaf0f5a30827722f26f276a64019c24fab05d0b22cb20cef5cbc17ec7d9bf30f",
        blockNumber: 4812273,
        type: "deposit_dao" as TransactionType,
        amount: 2000,
        timestamp: new Date("2022-03-24T14:28:12.771Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x1ac185a0ad7bb3ad02bf2ed99c0fd5c3d528ac21dc657620065dfd727d470f31",
        inputs: [],
        outputs: [],
        blockHash: "0x70fdbc559b3517a9682f555fe5340fd725baab5104f7fcc7c33bfafc37854741",
        blockNumber: 4812409,
        type: "receive_ckb" as TransactionType,
        amount: 10000,
        timestamp: new Date("2022-03-24T14:46:42.237Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xe53b34a7a670fd634173039cb38a9c70b1a2405870db5730375c7534c5af1d26",
        inputs: [],
        outputs: [],
        blockHash: "0xc3df9c214b728a3231a35625b6e374401230d05b8f978a2b97d17c3793c5b545",
        blockNumber: 4813293,
        type: "send_ckb" as TransactionType,
        amount: 10250,
        timestamp: new Date("2022-03-24T16:40:18.414Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x94e11b622743fbcd9261b9f509d9bd9b121fffb6faac9b9fb582e45da2319614",
        inputs: [],
        outputs: [],
        blockHash: "0x711be54800cdbf6c1607c8098b276fadf7ead000d06cce1489d0ff5f35e8b291",
        blockNumber: 4887015,
        type: "smart_contract" as TransactionType,
        amount: 150,
        timestamp: new Date("2022-03-31T12:37:41.173Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x81bbc439433bc380f6910752bb2ed1ed3bc520d0d7678376a1baf3dd571649b2",
        inputs: [],
        outputs: [],
        blockHash: "0x89ff1fad43de1ea3e42961215224755478a9fc08033bc1f17bfed0207ad989ef",
        blockNumber: 4724117,
        type: "receive_ckb" as TransactionType,
        amount: 10000,
        timestamp: new Date("2022-03-16T10:32:34.819Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x9469fc7c6991197ee6f370244402b75cba78360fa4d553ad082fbb0dae2d7626",
        inputs: [],
        outputs: [],
        blockHash: "0xdaf0f5a30827722f26f276a64019c24fab05d0b22cb20cef5cbc17ec7d9bf30f",
        blockNumber: 4812273,
        type: "deposit_dao" as TransactionType,
        amount: 2000,
        timestamp: new Date("2022-03-24T14:28:12.771Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xe53b34a7a670fd634173039cb38a9c70b1a2405870db5730375c7534c5af1d26",
        inputs: [],
        outputs: [],
        blockHash: "0xc3df9c214b728a3231a35625b6e374401230d05b8f978a2b97d17c3793c5b545",
        blockNumber: 4813293,
        type: "send_ckb" as TransactionType,
        amount: 10250,
        timestamp: new Date("2022-03-24T16:40:18.414Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xa43e1c45350cdcf16bfd0e5dda832302cf34300054f1ddeb571cf6ce178bb700",
        inputs: [],
        outputs: [],
        blockHash: "0x36d810d978e50426f52b29d36e3a1e7a9a1eb14a9ece2f30da16e9c81c61398b",
        blockNumber: 4821477,
        type: "send_ckb" as TransactionType,
        amount: 10250,
        timestamp: new Date("2022-03-25T10:59:05.094Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0x9469fc7c6991197ee6f370244402b75cba78360fa4d553ad082fbb0dae2d7626",
        inputs: [],
        outputs: [],
        blockHash: "0xdaf0f5a30827722f26f276a64019c24fab05d0b22cb20cef5cbc17ec7d9bf30f",
        blockNumber: 4812273,
        type: "deposit_dao" as TransactionType,
        amount: 2000,
        timestamp: new Date("2022-03-24T14:28:12.771Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xe53b34a7a670fd634173039cb38a9c70b1a2405870db5730375c7534c5af1d26",
        inputs: [],
        outputs: [],
        blockHash: "0xc3df9c214b728a3231a35625b6e374401230d05b8f978a2b97d17c3793c5b545",
        blockNumber: 4813293,
        type: "send_ckb" as TransactionType,
        amount: 10250,
        timestamp: new Date("2022-03-24T16:40:18.414Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xa43e1c45350cdcf16bfd0e5dda832302cf34300054f1ddeb571cf6ce178bb700",
        inputs: [],
        outputs: [],
        blockHash: "0x36d810d978e50426f52b29d36e3a1e7a9a1eb14a9ece2f30da16e9c81c61398b",
        blockNumber: 4821477,
        type: "send_ckb" as TransactionType,
        amount: 10250,
        timestamp: new Date("2022-03-25T10:59:05.094Z"),
    },
    {
        status: "committed" as TransactionStatus,
        transactionHash: "0xa43e1c45350cdcf16bfd0e5dda832302cf34300054f1ddeb571cf6ce178bb700",
        inputs: [],
        outputs: [],
        blockHash: "0x36d810d978e50426f52b29d36e3a1e7a9a1eb14a9ece2f30da16e9c81c61398b",
        blockNumber: 4821477,
        type: "send_ckb" as TransactionType,
        amount: 10250,
        timestamp: new Date("2022-03-25T10:59:05.094Z"),
    },
];
